...
#Best working value for lambda_g: 0.0001
lambda_g = 0.000000001
#Best working value for lambda_e: 0.0005
lambda_e = 0.0000003
...
sigma = 0.05
R = 8.314
#Tentative c_g value: 0.01
c_g = 0.01
v_w = 1.8054197671114737e-05
#const = 1e6
const = 1e3
#upper bound on const2 = 200000; lower bound = 0; best working value: 10000
const2 = 9
#Best working value of c_e so far: 1.0e5...9.0e5
c_e = 3.0e6
...
def P_e(t,w_a,T_e):
    ...
    return ((lambda_g*(d(T_e) + b(w_a,T_e)*f(w_a,T_e) + c(T_e)*f(w_a,T_e) + l(w_a,T_e)*const2*np.exp(l(w_a,T_e)*t*lambda_e)) + lambda_e*l(w_a,T_e)*(d(T_e) + b(w_a,T_e)*f(w_a,T_e) +c(T_e)*f(w_a,T_e) + const*f(w_a,T_e)*np.exp(-lambda_g*t) + l(w_a,T_e)*const2*np.exp(l(w_a,T_e)*t*lambda_e)))/((lambda_g + l(w_a,T_e)*lambda_e)*l(w_a,T_e)))*(10e-6)